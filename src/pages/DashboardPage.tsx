...
import {
  Calendar, Clock, Hash, User, FileText, Activity, Heart,
  ChevronRight, Download, Eye, Filter, Search, Bell, Shield,
  Pill, Thermometer, Droplets, TrendingUp
} from "lucide-react";
...
const activeTokens = [
  { number: 3, department: "Cardiology", doctor: "Dr. Anil Kapoor", status: "waiting" as const, position: 2, estimatedTime: "~20 min" },
  { number: 7, department: "General Medicine", doctor: "Dr. Pooja Reddy", status: "in-progress" as const, position: 0, estimatedTime: "Now" },
];

const appointments = [
  { id: "APT-001", department: "Cardiology", doctor: "Dr. Anil Kapoor", date: "2026-02-25", time: "10:00 AM", status: "upcoming" as const },
  { id: "APT-002", department: "Neurology", doctor: "Dr. Rajesh Gupta", date: "2026-02-20", time: "11:30 AM", status: "completed" as const },
  { id: "APT-003", department: "General Medicine", doctor: "Dr. Pooja Reddy", date: "2026-02-15", time: "09:00 AM", status: "completed" as const },
  { id: "APT-004", department: "Orthopedics", doctor: "Dr. Vikram Rao", date: "2026-01-28", time: "02:30 PM", status: "completed" as const },
  { id: "APT-005", department: "Pediatrics", doctor: "Dr. Kavita Sharma", date: "2026-01-10", time: "03:00 PM", status: "cancelled" as const },
];
...
const vitals = [
  { label: "Blood Pressure", value: "120/80", unit: "mmHg", icon: Heart, trend: "normal" as const },
  { label: "Heart Rate", value: "72", unit: "bpm", icon: Activity, trend: "normal" as const },
  { label: "Temperature", value: "98.6", unit: "°F", icon: Thermometer, trend: "normal" as const },
  { label: "Blood Sugar", value: "110", unit: "mg/dL", icon: Droplets, trend: "up" as const },
];
...
const recordTypeConfig: Record<string, string> = {
          ...
          >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {activeTokens.map((token, i) => (
                <motion.div key={token.number} custom={i} initial="hidden" animate="visible" variants={fadeUp}
          ...
          <TabsContent value="appointments" className="space-y-4">
            ...
            <div className="space-y-3">
              {filteredApts.map((apt, i) => {
                ...
                return (
                  <motion.div key={apt.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
          ...
          <TabsContent value="records" className="space-y-4">
            ...
            <div className="space-y-3">
              {filteredRecords.map((rec, i) => (
                <motion.div key={rec.id} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
                  className="bg-card border border-border rounded-xl p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:shadow-medium transition-shadow"
                >
          ...
          <TabsContent value="prescriptions" className="space-y-4">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {prescriptions.map((rx, i) => (
                <motion.div key={rx.name} custom={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}
